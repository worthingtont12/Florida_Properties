...
#missmap(df_known)
lm1 <- lm(diff_btwn_prc_jv ~
          lnd_sqfoot
          +
          no_buldng
          +
          const_class_explained
          +
          vi_cd1_explained
          +
          sale_season1
...
df_known$overvsunder <- ifelse(df_known$diff_btwn_prc_jv, 1, 0)
regsubsets.out <-
  regsubsets(overvsunder ~ bas_strt+pa_uc+lnd_sqfoot+no_buldng+const_class_explained+vi_cd1_explained+sale_season1+act_age+Blk_Val+TOT_POP+TOT_MED_AGE+PCT_M+M_MED_AGE+PCT_F+F_MED_AGE+AVG_HOUSE_SIZE+AVG_FAM_SIZE+HOME_VAC_RATE+RENT_VAC_RATE+AVG_H_SIZE_RENTER+PCT_UNEMP+TOT_FAM+PCT_FAM_POV+TOT_FAM_MARRIED+PCT_FAM_MARRIED_POV + TOT_FAM_F_ONLY+PCT_FAM_F_ONLY_POV+POV_RATE_NO_GED+POV_RATE_SOME_COL+POV_RATE_BACH_PLUS+PCT_H+PCT_WA+PCT_BA+PCT_AA+PCT_OTHER+PCT_TOT_POP_18+PCT_VAC_HOUSE+PCT_FAM+PCT_FAM_MARRIED,
             data = df_known,
             nbest = 1,       # 1 best model for each number of predictors
             nvmax = NULL,    # NULL for no limit on number of variables
             force.in = NULL, force.out = NULL,
...
logit1 <- glm(y ~ x, data = df_known, family="binomial")
